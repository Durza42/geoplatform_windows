 ...
 */
   ...
   /* traite les evenement. retourne false si il faut quitter, true si le jour veut continuer. */
...
*/
      Mix_PlayMusic (gal_mus -> Musique_cutted [var -> lvl - 1], 1);
   }
   var -> itt = 0;

   SDL_TimerID timer; // Variable pour stocker le numéro du timer
   timer = SDL_AddTimer (15, thread, var);
   ...
   while (1) {
      ...
      while (SDL_PollEvent (&event)) { // il y a eu un evenement depuis la derniere fois
         if (event.type == SDL_WINDOWEVENT) { // une touche est pessée
            ...
            if (event.key.keysym.sym == SDLK_LSHIFT) { // on diminue la musique

               if (gal_mus -> volume - 10 >= 0)
                  gal_mus -> volume -= 10;
               else
                  gal_mus -> volume = 0;

               Mix_VolumeMusic (gal_mus -> volume);
            }
            if (event.key.keysym.sym == SDLK_RSHIFT) { // on diminue la musique

               if (gal_mus -> volume + 10 <= MIX_MAX_VOLUME)
                  gal_mus -> volume += 10;
               else
...
void refresh (Var *var, Gal *gal) {
   ...
   /* thread qui se charge d'appeler à interval régulier la fonction qui décale les sprites vers la gauche */
Uint32 thread (Uint32 intervalle, void *data) {
...
void decaler_sprites (Var *var, int itt) {
